################################################################################
#   SCRIPT:     2_est_nmnl_main.jl
#   AUTHOR:     Jacob Bradt (jacob.bradt@mccombs.utexas.edu)
#   NOTES:      julia script to run main estimates reported in Table 3 of 
...
θ_hat1, θ_hat1_boot = est_nmnl_full(choi, cost, [-1.1, 0.5], maxiter = 500, n_boot = n_boot);

#   Save results to file:
CSV.write(
    "data/dwh_replication/est/est_nmnl_main1.csv", 
    DataFrame(θ_hat1, :auto), 
    writeheader=false
);
CSV.write(
    "data/dwh_replication/boot_est/boot_est_nmnl_main1.csv", 
    DataFrame(θ_hat1_boot, :auto), 
    writeheader=false
);

#   Import results from file, if already estimated:
#θ_hat1 = readdlm("data/est/est_nmnl_main1.csv", ',', header = false)[:, 1];
#θ_hat1_boot = readdlm("data/boot_est/boot_est_nmnl_main1.csv", ',', header = false);
...
#   Nested logit w/ CF:
θ_hat2, θ_hat2_boot = est_nmnl_full(choi, cost, [-1.0, 0.0, 0.9], Z = Z_full, maxiter = 500, n_boot = n_boot);

#   Save results to file:
CSV.write(
    "data/dwh_replication/est/est_nmnl_main2.csv", 
    DataFrame(θ_hat2, :auto), 
    writeheader=false
);
CSV.write(
    "data/dwh_replication/boot_est/boot_est_nmnl_main2.csv", 
    DataFrame(θ_hat2_boot, :auto), 
    writeheader=false
);

#   Import results from file, if already estimated:
#θ_hat2 = readdlm("data/est/est_nmnl_main2.csv", ',', header = false)[:, 1];
#θ_hat2_boot = readdlm("data/boot_est/boot_est_nmnl_main2.csv", ',', header = false);
...
open("output/dwh_replication/dwh_second_stage_main.tex", "w") do file
    write(file, t_tex)
end;
