############################## FRONT MATTER ##############################
#   SCRIPT:     gen_instrument.R
#   AUTHOR:     Jacob Bradt (jbradt@g.harvard.edu)
#   NOTES:      Script to construct instrument and control function for
...
# Set project root:
i_am("R/first_stage.R")

# STEP 1: Import distance matrix ----

# Import distance matrix:
origins <- fread(here("data/distance_mat.csv"))

# STEP 2: Import and merge travel cost data ----

# Travel cost data:
tc <- fread(here("data/tc1.txt"))
...
# Import WTI data:
wti <- fread(here("data/eia_wti.csv"))
...
supply_shocks <- fread(here("data/oil_struc_shocks.csv"))
...
respondents_commute <- fread(here("data/respondents_commutes.csv"))

# Add CBSA:
cbsa <- fread(here("data/ZIP_CBSA_092012.csv"))
...
gas_tax <- fread(here("data/gas_tax_delta.csv"))
...
acs_2012 <- fread(here("data/acs5yr_2012.csv"))
...
etable(
  ...
  file = here("output/dwh_first_stage_main.tex"),
  replace = T
)
etable(
  fs_local,
  ...
  file = here("output/dwh_first_stage_local.tex"),
  replace = T
)
