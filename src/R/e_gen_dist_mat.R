...
# Load dependencies:
pacman::p_load(here, data.table, tidyverse, fixest, sf, units, zoo, fredr, tidycensus, tigris, ggspatial, cowplot, prettymapr, raster)

# Set project root:
i_am("src/R/e_gen_dist_mat.R")
...
origins_local <- fread(here("data/dwh_replication/origins_local.csv"))
...
origins_national <- fread(here("data/dwh_replication/origins_national.csv"))
...
fwrite(origins, here("data/dwh_replication/distance_mat.csv"))
...
ggsave(here( "output/dwh_replication/fig_dist_freq.jpg"), plot = fig_dist_freq, width = 4, height = 4, units = "in", dpi = 500)
...
fig_site_map <- ggplot(data = sites_st) +
  ...
  theme(legend.title = element_blank(),
        legend.position.inside = c(0.275, 0.225), 
        legend.background = element_rect(fill = "transparent"),
        legend.key = element_rect(fill = "transparent"))
ggsave(here( "output/dwh_replication/fig_site_map.jpg"), plot = fig_site_map, width = 6, height = 4, units = "in", dpi = 500)
